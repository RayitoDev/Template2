...
function createShadows(colorChannel: string, mode: 'light' | 'dark' = 'light'): Shadows {   
    const lightColor1 = varAlpha(colorChannel, 0.2);
    const lightColor2 = varAlpha(colorChannel, 0.14);
    const lightColor3 = varAlpha(colorChannel, 0.12);

    const darkColor1 = varAlpha(colorChannel, 0.3);
    const darkColor2 = varAlpha(colorChannel, 0.2);
    const darkColor3 = varAlpha(colorChannel, 0.18);

    const color1 = mode === 'light' ? lightColor1 : darkColor1;
    const color2 = mode === 'light' ? lightColor2 : darkColor2;
    const color3 = mode === 'light' ? lightColor3 : darkColor3;
